  ...
  <div class="w-full lg:w-1/2">
    <div class="bg-(--panel) shadow-sm rounded-2xl p-6 border border-(--border-strong)">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h5 class="font-semibold text-base md:text-lg mb-0 text-(--text)">Tambah Tahun</h5>
          <div class="text-(--muted) text-xs md:text-sm">Masukkan tahun 4 digit (contoh: 2026)</div>
        </div>
        <i class="bi bi-calendar-plus text-2xl md:text-3xl text-(--muted) opacity-50"></i>
      </div>
      ...
      <form method="POST" action="{{ route('tahun.store') }}">
        ...
        <div class="mb-4">
          ...
          <input type="text" name="tahun"
                 ...
                 value="{{ old('tahun') }}" inputmode="numeric" pattern="[0-9]*" maxlength="4" placeholder="2026" required>
        ...
        <div class="flex justify-between mt-8">
          ...
          <button type="submit" class="px-3 md:px-4 py-1.5 md:py-2 bg-(--brand) text-white rounded-xl hover:opacity-90 flex items-center gap-2 transition-opacity">
            <i class="bi bi-save"></i> Simpan Tahun
          </button>
        </div>
      </form>
    </div>
