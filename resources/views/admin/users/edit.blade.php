...
<div class="profile-container mx-auto">
  <div class="flex items-center justify-between mb-4">
    <div>
      <h2 style="font-size: 1.5rem; font-weight: 700; margin: 0;">Edit User</h2>
      <p class="profile-subheader mb-0">Ubah data user (ADMIN, OPD, atau BPS).</p>
    </div>
  ...
  <div class="profile-card">
    ...
    <form method="POST" action="{{ route('users.update', $user->id) }}">
      ...
      <div class="mb-4 flex items-center">
        ...
        <div class="flex-grow max-w-md w-full ml-4">
          <input type="text" class="form-control w-full @error('username') is-invalid @enderror" name="username" value="{{ old('username', $user->username) }}" required />
          @error('username')
            <div class="invalid-feedback d-block mt-1">{{ $message }}</div>
          @enderror
        </div>
      ...
      <div class="mb-4 flex items-center">
        ...
        <div class="flex-grow max-w-md w-full ml-4">
          <select name="role" class="form-select w-full @error('role') is-invalid @enderror" required>
            ...
            <option value="bps" {{ old('role', $user->role)=='bps'?'selected':'' }}>BPS</option>
          </select>
          @error('role')
            <div class="invalid-feedback d-block mt-1">{{ $message }}</div>
          @enderror
      ...
      <div class="card-title mt-6 flex justify-between items-center w-full">
        <div>
          <i class="bi bi-shield-lock"></i>
          Pengaturan Password <span class="text-sm font-normal text-(--muted) ml-2 border border-blue-500/30 bg-blue-500/10 text-blue-500 px-2 rounded-full py-0.5" style="border-radius: 9999px;">Opsional</span>
        </div>
      </div>

      <p class="text-sm text-(--muted) mb-4 mt-1 border-l-2 border-amber-500/50 pl-3">Kosongkan kolom ini jika password pengguna tidak ingin diubah.</p>
      ...
      <div class="mb-4 flex flex-col md:flex-row md:items-start gap-2 md:gap-4 mt-4">
        ...
        <div class="flex-grow max-w-md w-full md:ml-4 mt-1">
          <div class="flex">
            <input type="password" id="passwordBaru" class="form-control w-full rounded-r-none border-r-0 focus:z-10 @error('password') is-invalid @enderror" name="password" placeholder="Kosongkan jika tidak diubah" />
          ...
          <div class="password-requirement mt-3" id="pwdReqBox" style="display: none;">
      ...
      <div class="mb-4 flex flex-col md:flex-row md:items-start gap-2 md:gap-4">
        ...
        <div class="flex-grow max-w-md w-full md:ml-4 mt-1">
          <div class="flex">
            <input type="password" id="passwordBaruConfirmation" class="form-control w-full rounded-r-none border-r-0 focus:z-10 @error('password_confirmation') is-invalid @enderror" name="password_confirmation" placeholder="Ulangi password baru" />
...
<script>
  ...
  const pwdReqBox = document.getElementById('pwdReqBox');
  const requirements = {
  ...
  if (passwordInput) {
    passwordInput.addEventListener('input', function () {
      ...
      // Jika kosong, hilangkan requirement box
      if (pwd === '') {
