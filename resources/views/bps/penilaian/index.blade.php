...
<div class="bg-(--panel) border border-(--border-strong) rounded-2xl p-4 mb-6">
  <form method="GET" action="{{ route('bps.penilaian.index') }}" class="flex flex-col sm:flex-row flex-wrap gap-3 items-start sm:items-end">
    ...
    <div class="flex flex-wrap gap-2 shrink-0">
      ...
      <a class="px-3 py-2 bg-green-600 border border-green-500 text-white rounded-xl hover:bg-green-700 transition-colors flex items-center gap-2 text-xs md:text-sm"
         href="{{ route('bps.penilaian.export', ['user_id' => $userId, 'export_year' => $exportYear ?? 0]) }}">
